...
if ($is_editor) {
  ...
  if (isset($thisAnnouncementId) && $thisAnnouncementId && isset($sortDirection) && $sortDirection) {
	...
	$fields = array('message' => urlencode(str_replace('#',' #',strip_tags($_POST['newContent']))),
			'link' => urlencode('localhost/openeclass/modules/announcements/index.php?course=TMAPOST100&an_id=16'));
	...
	curl_setopt($ch,CURLOPT_POST, count($fields));//kanei post mono otan count($fields)==TRUE pou einai mias kai count($fileds)>=1 otan grafoume kati sto keimeno tis anakoinwsis
	curl_setopt($ch,CURLOPT_POSTFIELDS, $fields_string);
	curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);//min typwsiseis stin othoni alla vale ta apotelesmata stin $result parakatw
	//execute post
	$result = curl_exec($ch);
	//close connection
	curl_close($ch);
